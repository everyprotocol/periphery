...
import {Descriptor, SetFullFeatured} from "@everyprotocol/periphery/SetFullFeatured.sol";
...
contract MySetFullFeatured is SetFullFeatured {
    constructor(address owner, address minter, address setr, uint64 kindId, uint32 kindRev)
        SetFullFeatured(owner, minter, setr, kindId, kindRev)
    {}
}

contract SetFullFeatured_Test is Test {
    // Protocol roles
    ...
    function setUp() public {
        vm.mockCall(setRegistry, abi.encodeWithSignature("omniRegistry()"), abi.encode(makeAddr("omniRegistry")));
        vm.mockCall(setRegistry, abi.encodeWithSignature("kindRegistry()"), abi.encode(makeAddr("kindRegistry")));
        vm.mockCall(setRegistry, abi.encodeWithSignature("elementRegistry()"), abi.encode(makeAddr("elementRegistry")));

        set = new MySetFullFeatured(setOwner, minter, setRegistry, KIND_ID, KIND_REV);
    }
    ...
    function test_RemoteMint() public {
        ...
        assertEq(set.owner(id), holder, "Holder should own the minted object");
    }
    ...
    function test_RemoteMintRevertOnZeroId() public {
        ...
        vm.prank(minter);
        vm.expectRevert(SetFullFeatured.ObjectIdUnspecified.selector);
    ...
    function test_ERC1155Compatibility() public {
        ...
        assertEq(set.owner(id), caller, "Ownership should be transferred");
    ...
    function test_ObjectRelationCallbacks() public {
        ...
        vm.mockCall(setRegistry, abi.encodeWithSignature("omniRegistry()"), abi.encode(omniRegistry));
    ...
    function test_URI() public {
        ...
        assertEq(baseUri, "https://example.com/setfullfeatured/{id}/{rev}/meta", "Base URI should match");
        ...
        assertEq(tokenUri, "https://example.com/setfullfeatured/42/0/meta", "Token URI should match");

        // Test contract URI
        string memory contractUri = set.contractURI();
        assertEq(contractUri, "https://example.com/setfullfeatured/meta", "Contract URI should match");
    }
}
